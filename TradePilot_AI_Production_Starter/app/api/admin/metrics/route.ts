...
export async function GET() {
  try {
    ...
    /*
    |--------------------------------------------------------------------------
    | Load all Supabase Auth users
    |--------------------------------------------------------------------------
    |
    | This helper lives inside GET so TypeScript uses the exact inferred type
    | of the Supabase admin client created above.
    |
    */
    ...
    const activeStripeSubscriptions =
      stripeSubscriptions.filter(
        (subscription) =>
          subscription.status ===
            "active" ||
          subscription.status ===
    ...
    const stripeMrr =
      activeStripeSubscriptions.reduce(
        (
          total,
          subscription
        ) =>
...
async function listAllStripeCharges(
  stripe: Stripe
) {
  const charges:
    Stripe.Charge[] =
...
function subscriptionMonthlyValue(
  subscription:
    Stripe.Subscription
) {
